    ...
    for (var i = 0; i < resultsCache.length; i++) {
        ...
        if (result.summary.passed) {
            md.appendTableRow([
                ...
                result.summary.tests["Edge"].passed,
                result.summary.tests["PluginFree"].passed,
                result.summary.tests["JavaScriptLibraries"].passed,
                result.summary.tests["CSSPrefixes"].passed,
                result.summary.tests["BrowserDetection"].passed,
            ]);
        }
    }
    md.appendLine();
